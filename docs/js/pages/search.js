JYS.Pages.search = function(params) {
  ...
  function splitHighlight(text, keyword) {
    ...
    parts.forEach(function(part) {
      ...
      segments.push({ text: part, highlight: part.toLowerCase() === keyword.toLowerCase() });
    });
    return segments.length > 0 ? segments : [{ text: text, highlight: false }];
  }
  ...
  return {
    ...
    onRender: function() {
      ...
      var debouncedSearch = U.debounce(function() {
        ...
        else { searchTips.style.display = ''; searchResults.style.display = 'none'; }
      }, 300);
      ...
      searchInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') { var kw = searchInput.value.trim(); if (kw) doSearch(kw); }
      });
      ...
      function doSearch(keyword) {
        ...
        var charResults = characters.filter(function(c) {
          return c.name.toLowerCase().indexOf(kw) !== -1 || (c.nickname && c.nickname.toLowerCase().indexOf(kw) !== -1);
        }).map(function(c) {
          return { id: c.id, name: c.name, avatar: c.avatar, contentCount: c.contentCount, nameSegments: splitHighlight(c.name, kw) };
        });

        var contentResults = contents.filter(function(c) {
          return (c.text && c.text.toLowerCase().indexOf(kw) !== -1) || (c.tags && c.tags.some(function(t) { return t.indexOf(kw) !== -1; }));
        }).map(function(c) {
          ...
          return {
            ...
            text: c.text, images: c.images || [], tags: c.tags || [],
            formattedTime: U.formatTime(c.createdAt),
            textSegments: c.text ? splitHighlight(c.text, kw) : []
          };
        });
        ...
        if (charResults.length > 0) {
          charResults.forEach(function(c) {
            charHTML +=
              ...
              '</div>' +
              '<div class="char-info">' +
              '<span class="char-name">' + c.nameSegments.map(function(s) { return '<span class="' + (s.highlight ? 'highlight' : '') + '">' + U.escapeHtml(s.text) + '</span>'; }).join('') + '</span>' +
        ...
        if (contentResults.length > 0) {
          contentResults.forEach(function(item) {
            contHTML +=
              ...
              '</div>';
            if (item.text) {
              contHTML += '<div class="item-text">' + item.textSegments.map(function(s) { return '<span class="' + (s.highlight ? 'highlight' : '') + '">' + U.escapeHtml(s.text) + '</span>'; }).join('') + '</div>';
            }
        ...
        document.querySelectorAll('.char-result-item').forEach(function(el) {
          el.addEventListener('click', function() { JYS.App.navigateTo('/character/detail?id=' + el.dataset.id); });
        });
        document.querySelectorAll('.result-item').forEach(function(el) {
          el.addEventListener('click', function() { JYS.App.navigateTo('/content/edit?id=' + el.dataset.id + '&characterId=' + el.dataset.charId); });
        });
